enum chopper_attachments {
	MONSTER_HUMAN_CHOPPER = 0,
	MONSTER_HUMAN_CHOPPER_BOMB = 1
};

class monster_human_chopper : ScriptBaseMonsterEntity
{
	string[] szArrayIdleSound = {
		"gunmanchronicles/apache/ap_rotor2.wav"
	};
	
	string[] szArrayApproachSound = {
		"gunmanchronicles/apache/approach.wav"
	};
	...
	void Spawn()
	{
        ...
        if( string( self.pev.model ).IsEmpty() )
			self.pev.model = "models/gunmanchronicles/chopper.mdl";
        ...
        if( string( self.pev.netname ).IsEmpty() )
			self.pev.netname = "Chopper Gunman";
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "models/gunmanchronicles/chopper.mdl" );
		m_iGibModelindex   = g_Game.PrecacheModel( "models/gunmanchronicles/choppergibs.mdl" );
		...
		for( uint i = 0; i < szArrayApproachSound.length(); i++ )
			g_SoundSystem.PrecacheSound( szArrayApproachSound[i] );
	...
	*/
	...
	//=========================================================
	// Approach Sound - Check where is this used; KCM 
	//=========================================================
	/*void ApproachSound()
	{
		g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_STATIC, szArrayApproachSound[ Math.RandomLong( 0, szArrayApproachSound.length()-1 ) ], 1.0, ATTN_NORM, 0, PITCH_NORM );
	}*/
...
void RegisterEntity_MonsterHumanChopper()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "monster_human_chopper", "monster_human_chopper" );
}
